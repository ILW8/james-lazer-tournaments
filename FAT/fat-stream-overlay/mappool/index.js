...
obsGetScenes((scenes) => {
  ...
  obsGetCurrentScene((scene) => {
    document
      .getElementById(`scene__${scene.name}`)
      .classList.add('activeScene');
  });
});
...
class Beatmap {
  ...
  generate() {
    const identifier = this.beatmap.identifier.toLowerCase();
    this.parent = $('<div></div>')
      .addClass('map')
      .attr('id', `map-${identifier}`);
    this.parent.append(
      $('<div></div>')
        .addClass('map-image')
        .css(
    ...
    this.mod_icon = $('<div></div>')
      ...
      .text(identifier.replace('host', 'h').toUpperCase());
    ...
    const mods = identifier.startsWith('host')
      ? 'ez'
      : this.beatmap.mods.toLowerCase();
    $(`#mod-container-${mods}`).append(this.parent);
  }
}
...
async function setupBeatmaps() {
  ...
  const mods = [...new Set(maps.map((b) => b.mods))];
    ...
    bm.parent.on('click', (event) => {
      if (!event.originalEvent.shiftKey)
        event.originalEvent.ctrlKey
          ? banMap(bm, redName, 'red')
          : pickMap(bm, redName, 'red');
      else resetMap(bm);
    });
    bm.parent.on('contextmenu', (event) => {
      if (!event.originalEvent.shiftKey)
        event.originalEvent.ctrlKey
          ? banMap(bm, blueName, 'blue')
          : pickMap(bm, blueName, 'blue');
      else resetMap(bm);
    });
    beatmaps.add(bm);
  }
...
const switchPick = (color) => {
  ...
  $('#current_pick')
    .text(`${currentPicker.toUpperCase()} PICK`)
    .addClass(currentPicker)
    .removeClass(opposite_team(currentPicker));
};
