...
let animation = {
    red_score: new CountUp('score-red', 0, 0, 0, .3, {
        useEasing: true,
        useGrouping: true,
        separator: ',',
        decimal: '.',
    ...
    blue_score: new CountUp('score-blue', 0, 0, 0, .3, {
        useEasing: true,
        useGrouping: true,
        separator: ',',
        decimal: '.',
    ...
    score_diff: new CountUp('scorediff', 0, 0, 0, .3, {
        useEasing: true,
        useGrouping: true,
        separator: ',',
        decimal: '.',
...
socket.onmessage = event => {
    ...
    if (data.type === "Beatmap") {
        ...
        let b = JSON.parse(JSON.stringify(data.message));
        delete b['beatmap_set']
        // console.log(JSON.stringify(b, null, 2));

        // update background image
        image = `https://assets.ppy.sh/beatmaps/${data.message.beatmap_set.online_id}/covers/cover.jpg`;
        image_container.style.backgroundImage = `url('${image}')`;
        strain_background.style.backgroundImage = `url('${image}')`;

    	// update title
        title_ = `${data.message.metadata.artist} - ${data.message.metadata.title}`;
        title.innerHTML = title_;

        // update diff/mapper
        diff_ = data.message.difficulty_name;
        diff.innerHTML = `[${diff_}]`;
        mapper.innerHTML = data.message.metadata.author.username;

        // update map stats
		cs.innerHTML =  Math.round(data.message.difficulty.circle_size * 10) / 10;
        ar.innerHTML = Math.round(data.message.difficulty.approach_rate * 10) / 10;
		od.innerHTML =  Math.round(data.message.difficulty.overall_difficulty * 10) / 10;
		sr.innerHTML = `${Math.round(data.message.star_rating * 100) / 100}★`;
		bpm.innerHTML = Math.round(data.message.bpm * 10) / 10;

        // update map drain time (length)
		// len_ = data.menu.bm.time.full - data.menu.bm.time.firstObj;
		len_ = data.message.length;
    ...
    if (data.type === "MultiplayerGameplay") {
        ...
        let scores = [];
        let a = Object.values(players)
        for (const [index, player] of a.entries()) {
            // console.log(player);
            let score = player.total_score;
            scores.push({id: index, score});
        }

        scoreRed = scores.filter(s => [0, 2, 4,].includes(s.id)).map(s => s.score).reduce((a, b) => a + b, 0);
        scoreBlue = scores.filter(s => [1, 3, 5,].includes(s.id)).map(s => s.score).reduce((a, b) => a + b, 0);
