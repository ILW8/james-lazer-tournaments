...
socket.onmessage = async event => {
	...
	if (cache.update_stats) {
		...
		if (map?.win_condition && map?.win_condition !== 'score') { $('#custom_wincon').text(map?.win_condition); $('#wincon').css('display', 'flex').css('opacity', 1); }
		else { $('#custom_wincon').text(''); $('#wincon').css('display', 'none').css('opacity', 0); }
